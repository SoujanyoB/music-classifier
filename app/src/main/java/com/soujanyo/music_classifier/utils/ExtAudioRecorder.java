...
public class ExtAudioRecorder {
        ...
        try {
            ...
            if (rUncompressed) { // RECORDING_UNCOMPRESSED
                ...
                if (bufferSize < AudioRecord.getMinBufferSize(sampleRate, channelConfig, audioFormat)) { // Check to make sure buffer size is not smaller than the smallest allowed one
                    ...
                    Log.w(ExtAudioRecorder.class.getName(), "Increasing buffer size to " + bufferSize);
                }
    ...
    public void prepare()
    {
        ...
        int RECORDER_BPP = 16;
        int RECORDER_SAMPLE_RATE = 22050;
        int RECORDER_CHANNELS = AudioFormat.CHANNEL_IN_STEREO;
            ...
            if (state == State.INITIALIZING)
            {
                if (rUncompressed)
                {
                    if ((audioRecorder.getState() == AudioRecord.STATE_INITIALIZED) & (filePath != null))
                    {
                        ...
                        randomAccessWriter.writeInt(Integer.reverseBytes(RECORDER_BPP)); // Sub-chunk size, 16 for PCM
